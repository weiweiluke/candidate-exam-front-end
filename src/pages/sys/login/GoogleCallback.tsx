...
import { useGoogleSignIn } from '@/store/userStore';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (code) {
        ...
        try {
          await googleSignIn({ code, state });
        } finally {
          setLoading(false);
        }
      } else {
        message.error('缺少必要的回调参数');
      }
    };
    fetchData();
  }, [location.hash]);
        ...
        <MotionContainer className="flex flex-col items-center justify-center px-2">
          <m.div variants={varBounce().in}>
            <Typography.Title level={3} className="text-center">
              Trying to log in to Google account, please wait...
            </Typography.Title>
          </m.div>
          ...
          <m.div variants={varBounce().in}>
            ...
            >
              ...
              <path
                ...
                opacity="0.2"
              />
              <image href={Character5} height="300" x="340" y="30" />
              <path
                fill={colorPrimary}
